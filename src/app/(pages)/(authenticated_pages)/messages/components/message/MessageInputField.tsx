...
interface SendMessageProps {
  ...
  typingData: string[];
  droppedFiles: File[];
  handleNewMsgEmail: (message: string) => void;
}
...
const MessageInputField: React.FC<SendMessageProps> = (
  ...
  const user = users[props.typingData[0]];
  const friendlyName = user?.friendlyName.split(" ")[0];
  const typingInfo = getTypingMessage(friendlyName ? [friendlyName] : props.typingData);
  ...
  const onMessageSend = async () => {
    ...
    props?.handleNewMsgEmail(message);
    setMessage("");
    setFiles([]);
