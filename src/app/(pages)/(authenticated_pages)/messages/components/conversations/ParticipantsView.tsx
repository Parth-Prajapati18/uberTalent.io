...
const ParticipantsView: React.FC<ParticipantsViewProps> = (
  ...
  const displayedParticipants: {name: string, identity?: string}[] = [];
  ...
  for (let i = 0; i < props.participants.length; i++) {
    ...
    if (i < maxDisplayedParticipants) {
      displayedParticipants.push({name, identity: participant.identity || ""});
    } else {
  ...
  return (
    <>
      ...
      >
        ...
        <Button variant={"reset"} cursor="default">
          <AvatarGroup displayedParticipants={displayedParticipants} />
        </Button>
