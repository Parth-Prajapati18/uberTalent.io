...
export async function isParticipantOnline(client: Client, identity: string ): Promise<boolean> {
  const user: User = await client.getUser(identity);
...
export async function getToken(): Promise<string> {
  console.log("getToken for twillo Service");
  const requestAddress = "/api/user/twilo/token";
  // const requestAddress = process.env
  //   .REACT_APP_ACCESS_TOKEN_SERVICE_URL as string;
  if (!requestAddress) {
    throw new Error(
